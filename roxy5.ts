...
// 因为要有 onMount 事件执行，所以还是 上面两种 mount(<App />, dom) 使用方式更好
render(<App />).insert(document.querySelector('#app'));
render(<App />).update(document.querySelector('#app'));
render(<App />).create();

// 尽管这里是 div.ref 但框架本身没有加响应式。倒是可以有其他的响应式库是 const a = ref(0); a.ref = 123; 这样
const div = render(() => <div />);
onMounted(() => div.ref.clientWidth);
return () => <div>{div}</div> // 这种不能往 div 里传参

const div = render(() => <div />);
onMounted(() => div.ref[0].clientWidth);
return () => <div>{list.map(_ => div)}</div>  // key 怎么办。。。

// vs
const div = render(() => <div />);
onMounted(() => div.ref.clientWidth);
return () => <div>{div.render()}</div>  // 这种可以往里面传参
// vs
const Div = render(() => <div />);
onMounted(() => Div.ref.clientWidth);
return () => <div><Div /></div> // 这种东西跟 临时组件 有什么区别呢？ 
// 组件是有自己的实例的概念的，而这种东西不应有自己的实例的概念，只能有自己渲染的东西的实例的概念，所以选择 div.render()

// Q：单个 vnode 对应多个实例要怎么理解
const vnode = <div />
return <div>{vnode}<br/>{vnode}</div>
// 需要知道这怎么理解是因为上面的 div.render() 可能有人写成
return () => { const a = div.render(); return <div>{a}{a}</div> } // 则 div.ref 到底是有两个实例，还是只有一个实例
// 这无法理解。因为 vnode 本身只是代表信息，它就是可以对应多份实例，这样导致处处都很麻烦。信息转化为实例就是在 框架内部实现的。
// ! 所以获取 ref 最合理的办法就是 const div_ref = e => div_ref.ref = e; <div ref={div_ref} />
